...
const displayName = computed(() => userEmail.value || t('dashLayout.defaultAccount'))
...
const activityNav = [
  {
    labelKey: 'dashLayout.nav.overview',
    to: '/dashboard',
  ...
  {
    labelKey: 'dashLayout.nav.deposits',
  ...
  {
    labelKey: 'dashLayout.nav.planning',
  ...
  {
    labelKey: 'dashLayout.nav.messages',
    to: '/dashboard/messages',
    icon: 'M216,48H40A16,16,0,0,0,24,64V224a15.85,15.85,0,0,0,9.24,14.5A16.13,16.13,0,0,0,40,240a15.89,15.89,0,0,0,10.25-3.78l.09-.07L83,208H216a16,16,0,0,0,16-16V64A16,16,0,0,0,216,48ZM216,192H80a8,8,0,0,0-5.23,1.95L40,224V64H216Z',
  },
  {
    labelKey: 'dashLayout.nav.notifications',
    to: '/dashboard/notifications',
    icon: 'M221.8,175.94C216.25,166.38,208,139.33,208,104a80,80,0,1,0-160,0c0,35.34-8.26,62.38-13.81,71.94A16,16,0,0,0,48,200H88.81a40,40,0,0,0,78.38,0H208a16,16,0,0,0,13.8-24.06ZM128,216a24,24,0,0,1-22.62-16h45.24A24,24,0,0,1,128,216Z',
  },
]

const discoverNav = [
  {
    labelKey: 'dashLayout.nav.catalog',
  ...
  {
    labelKey: 'dashLayout.nav.advice',
    to: '/dashboard/advice',
...
const profileNav = [
  ...
  {
    labelKey: 'dashLayout.nav.orders',
    to: '/dashboard/orders',
    ...
    <aside class="dashboard-sidebar">
      ...
      <nav class="dashboard-sidebar-nav">
        <div class="dashboard-sidebar-section">
          <span class="sidebar-section-title">{{ $t('dashLayout.sections.myActivity') }}</span>
          <ul class="sidebar-nav-list">
            <li v-for="item in activityNav" :key="item.to" class="sidebar-nav-item">
              <RouterLink
                :to="item.to"
                :data-tour="item.to === '/dashboard/listings' ? 'listings' : null"
              >
        ...
        <div class="dashboard-sidebar-section">
          <span class="sidebar-section-title">{{ $t('dashLayout.sections.discover') }}</span>
          <ul class="sidebar-nav-list">
            <li v-for="item in discoverNav" :key="item.to" class="sidebar-nav-item">
        ...
        <div class="dashboard-sidebar-section">
          <span class="sidebar-section-title">{{ $t('dashLayout.sections.myProfile') }}</span>
          <ul class="sidebar-nav-list">
            <li v-for="item in profileNav" :key="item.to" class="sidebar-nav-item">
              <RouterLink
                :to="item.to"
                :data-tour="item.to === '/dashboard/score' ? 'score' : null"
              >
                <svg class="sidebar-icon" viewBox="0 0 256 256" fill="currentColor">
                  <path :d="item.icon" />
                </svg>
