...
const events = ref<PEvent[]>([
  { id: 1, title: 'Atelier upcycling N1', kind: 'workshop', start: '2026-05-12 14:00', duration: 180, location: 'Atelier Paris 11' },
  { id: 2, title: 'Restaurer un meuble', kind: 'training', start: '2026-05-20 10:00', duration: 360, location: 'Atelier Paris 11' },
  { id: 3, title: 'Repair Café', kind: 'event', start: '2026-04-30 10:00', duration: 240, location: 'Atelier Paris 11' },
  { id: 4, title: 'Dépôt DEP-204', kind: 'deposit', start: '2026-04-29 16:00', duration: 30, location: 'Conteneur Bastille' },
])
  ...
  <DashboardLayout>
    <header class="dashboard-page-header">
      <div>
        ...
        <p class="muted measure">Vos formations, ateliers, événements et rendez-vous de dépôt à venir.</p>
      </div>
    ...
    <div class="stats-row">
      ...
      <div class="stat-tile">
        ...
        <span class="stat-tile-value">{{ events.filter((e) => e.start.startsWith('2026-05')).length }}</span>
      </div>
      <div class="stat-tile">
        ...
        <div class="layout-flex layout-columns layout-gap-small" style="margin-top: var(--space-3);">
          ...
          >
            ...
            <div class="planning-event-row-date">
              <span style="font-weight: 700; font-size: var(--font-size-xlarge);">{{ e.start.slice(8, 10) }}</span>
    ...
    <section v-else class="dashboard-card center" style="padding: var(--space-16);">
      <p class="muted">Vue agenda hebdomadaire (réutilise la grille `.planning-grid` du backoffice)</p>
    </section>

    <!-- Detail modal -->
    <AppModal :open="!!detailEvent" :title="detailEvent?.title" size="small" @close="detailEvent = null">
      <div v-if="detailEvent" class="layout-flex layout-columns layout-gap-medium">
        <div class="layout-flex layout-gap-small layout-items-center">
          <span class="badge">{{ kindLabel[detailEvent.kind] }}</span>
        </div>
        <div class="recap-row"><span class="tiny uppercase muted">Date</span><span>{{ detailEvent.start }}</span></div>
        <div class="recap-row"><span class="tiny uppercase muted">Durée</span><span>{{ detailEvent.duration }} min</span></div>
        <div class="recap-row"><span class="tiny uppercase muted">Lieu</span><span>{{ detailEvent.location }}</span></div>
      </div>
      <template #footer>
        <button class="ghost medium" style="color: var(--destructive-color);" @click="unsubscribe">Se désinscrire</button>
