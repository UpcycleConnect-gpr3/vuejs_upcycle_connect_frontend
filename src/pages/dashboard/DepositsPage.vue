...
const deposits = ref<Deposit[]>([
  { id: 1, reference: 'DEP-204', object: 'Lot vaisselle vintage', category: 'Vaisselle', intent: 'don', status: 'approved', code: '482917', barcode: 'UC-DEP-204-X9K2', createdAt: '2026-04-26' },
  { id: 2, reference: 'DEP-203', object: 'Mixeur cassé', category: 'Électroménager', intent: 'don', status: 'pending', code: null, barcode: null, createdAt: '2026-04-25' },
  { id: 3, reference: 'DEP-198', object: 'Bibliothèque IKEA', category: 'Mobilier', intent: 'vente', status: 'collected', code: '208451', barcode: 'UC-DEP-198-B3M9', createdAt: '2026-04-12' },
  { id: 4, reference: 'DEP-189', object: 'Sèche-cheveux HS', category: 'Électroménager', intent: 'don', status: 'rejected', code: null, barcode: null, createdAt: '2026-04-05' },
])
  ...
  <DashboardLayout>
    <header class="dashboard-page-header">
      <div>
        ...
        <h1>Mes dépôts</h1>
        <p class="muted measure">Demandez un dépôt, l'équipe Check valide et vous reçevez un code conteneur + un QR pour le retrait artisan.</p>
      </div>
      <button class="primary medium" @click="showNewModal = true">+ Nouvelle demande</button>
    </header>

    <div class="layout-flex layout-gap-small">
      <button class="forum-tab" :class="{ active: filter === 'active' }" @click="filter = 'active'">Actifs</button>
      <button class="forum-tab" :class="{ active: filter === 'all' }" @click="filter = 'all'">Tout · {{ deposits.length }}</button>
    </div>

    <div class="layout-flex layout-columns layout-gap-medium">
      <article v-for="d in filtered" :key="d.id" class="deposit-card">
        <div class="layout-flex layout-justify-between layout-items-start" style="flex-wrap: wrap; gap: var(--space-3);">
          <div class="layout-flex layout-columns" style="gap: 4px;">
            <div class="layout-flex layout-gap-small layout-items-center">
              ...
              <span class="badge" :class="statusMeta[d.status].badge">{{ statusMeta[d.status].label }}</span>
            </div>
            <h4>{{ d.object }}</h4>
    ...
    <AppModal :open="showNewModal" title="Nouvelle demande de dépôt" @close="showNewModal = false">
      <form class="layout-flex layout-columns layout-gap-medium" @submit.prevent="submit">
        <div class="form-group">
          ...
          <input v-model="form.object" type="text" class="primary medium full-width" placeholder="Ex: Lot vaisselle vintage" required />
        </div>
        ...
        <div class="form-group">
          ...
          <textarea v-model="form.description" class="primary full-width" rows="4" placeholder="Décrivez l'objet, son état, ses dimensions…"></textarea>
        </div>
        <p class="small muted">L'équipe Check vérifie sous 24h si l'objet peut être donné ou vendu, puis génère un code conteneur.</p>
      </form>
      <template #footer>
        <button class="ghost medium" @click="showNewModal = false">Annuler</button>
        <button class="primary medium" :disabled="!form.object || !form.category" @click="submit">Envoyer la demande</button>
      </template>
    </AppModal>
