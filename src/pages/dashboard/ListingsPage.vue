...
const listings = ref<Listing[]>([
  { id: 1, title: 'Table basse palette', type: 'don', category: 'Mobilier', price: null, status: 'approved', views: 142, createdAt: '2026-04-22', photoCount: 4 },
  { id: 2, title: 'Lot d\'outils main', type: 'vente', category: 'Outils', price: 35, status: 'pending', views: 0, createdAt: '2026-04-26', photoCount: 6 },
  { id: 3, title: 'Vélo enfant 16"', type: 'vente', category: 'Sport', price: 80, status: 'approved', views: 87, createdAt: '2026-04-20', photoCount: 3 },
  { id: 4, title: 'Vieille télé tube', type: 'don', category: 'Électronique', price: null, status: 'rejected', views: 0, createdAt: '2026-04-18', photoCount: 1 },
  { id: 5, title: 'Étagère bois massif', type: 'vente', category: 'Mobilier', price: 60, status: 'sold', views: 234, createdAt: '2026-04-10', photoCount: 5 },
])
  ...
  <DashboardLayout>
    <header class="dashboard-page-header">
      <div>
        ...
        <h1>Mes annonces</h1>
        <p class="muted measure">Déposez vos objets en don ou en vente. Validation par l'équipe sous 24-48h.</p>
      </div>
    ...
    <div class="layout-flex layout-gap-small" style="flex-wrap: wrap;">
      <button class="forum-tab" :class="{ active: filter === 'all' }" @click="filter = 'all'">Tout · {{ listings.length }}</button>
      <button v-for="(meta, key) in statusMeta" :key="key" class="forum-tab" :class="{ active: filter === key }" @click="filter = key as Status">
        {{ meta.label }}
      </button>
    </div>
      ...
      <article v-for="l in filtered" :key="l.id" class="listing-card">
        ...
        <div class="listing-body">
          <div class="layout-flex layout-gap-small layout-items-center" style="flex-wrap: wrap;">
            <span class="badge" :class="statusMeta[l.status].badge">{{ statusMeta[l.status].label }}</span>
          ...
          <div class="layout-flex layout-justify-between layout-items-end">
            <div>
              <div v-if="l.price" class="mono" style="font-size: var(--font-size-large); font-weight: 700;">{{ l.price }}€</div>
    ...
    <div v-if="filtered.length === 0" class="empty-state">
      ...
      <button class="primary medium" @click="showModal = true">+ Déposer ma première annonce</button>
    </div>
    ...
    <AppModal :open="showModal" size="medium" @close="close">
      ...
      <!-- Step 1 — Infos -->
      <form v-if="step === 1" class="layout-flex layout-columns layout-gap-medium" @submit.prevent="step = 2">
        <div class="form-group">
          <label class="uppercase">Type d'annonce</label>
          <div class="layout-flex layout-gap-small">
            <button type="button" class="forum-tab" :class="{ active: form.type === 'don' }" @click="form.type = 'don'">Don gratuit</button>
            <button type="button" class="forum-tab" :class="{ active: form.type === 'vente' }" @click="form.type = 'vente'">Vente</button>
          </div>
        </div>

        <div class="form-group">
          <label class="uppercase">Titre</label>
          <input v-model="form.title" type="text" class="primary medium full-width" placeholder="Ex: Table basse en palette restaurée" required />
        </div>
        ...
        <div class="layout-flex layout-gap-medium">
          ...
          <div v-if="form.type === 'vente'" class="form-group" style="width: 140px;">
            ...
            <input v-model.number="form.price" type="number" min="0" class="primary medium full-width" />
          </div>
        </div>

        <div class="form-group">
          <label class="uppercase">Description</label>
          <textarea v-model="form.description" class="primary full-width" rows="5" placeholder="Détaillez l'objet, son histoire, ses dimensions…" required></textarea>
        </div>
      </form>

      <!-- Step 2 — Photos -->
      <div v-else-if="step === 2" class="layout-flex layout-columns layout-gap-medium">
        <p class="muted">Ajoutez au moins une photo. Les annonces avec photos sont 5× plus consultées.</p>
      ...
      <div v-else-if="step === 3" class="layout-flex layout-columns layout-gap-medium">
        <div class="recap-row"><span class="tiny uppercase muted">Type</span><span>{{ form.type === 'don' ? 'Don gratuit' : `Vente · ${form.price}€` }}</span></div>
        <div class="recap-row"><span class="tiny uppercase muted">Titre</span><span>{{ form.title }}</span></div>
        <div class="recap-row"><span class="tiny uppercase muted">Catégorie</span><span>{{ form.category }}</span></div>
        <div class="recap-row"><span class="tiny uppercase muted">Photos</span><span>{{ form.photos.length }}</span></div>
        <p class="small muted">Votre annonce sera examinée par l'équipe. Vous serez notifié dès validation.</p>
      </div>
      ...
      <template #footer>
        ...
        <div class="layout-flex layout-gap-small">
          ...
          <button v-if="step < 3" class="primary medium" @click="step++" :disabled="step === 1 && !form.title">Suivant →</button>
