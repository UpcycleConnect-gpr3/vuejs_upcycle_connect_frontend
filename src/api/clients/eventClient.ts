...
export const getEvents = async (): Promise<UpcycleEvent[]> => {
  const { data } = await apiUpcycle.get<ApiResponse<UpcycleEvent[]>>(BASE_PATH)
  return data.data
}

export const getEventById = async (id: number): Promise<UpcycleEvent> => {
  const { data } = await apiUpcycle.get<ApiResponse<UpcycleEvent>>(`${BASE_PATH}/${id}`)
  return data.data
}

export const createEvent = async (payload: EventPayload): Promise<UpcycleEvent> => {
  const { data } = await apiUpcycle.post<ApiResponse<UpcycleEvent>>(BASE_PATH, payload)
  return data.data
}

export const updateEvent = async (id: number, payload: EventPayload): Promise<UpcycleEvent> => {
  const { data } = await apiUpcycle.put<ApiResponse<UpcycleEvent>>(`${BASE_PATH}/${id}`, payload)
  return data.data
}
...
export const createEventStep = async (
  ...
  const { data } = await apiUpcycle.post<ApiResponse<EventStep>>(
    `${BASE_PATH}/${id}/steps`,
    payload,
  )
  return data.data
