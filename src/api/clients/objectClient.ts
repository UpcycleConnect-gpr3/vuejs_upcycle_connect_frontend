...
export const getObjects = async (): Promise<UpcycleObject[]> => {
  const { data } = await apiUpcycle.get<ApiResponse<UpcycleObject[]>>(BASE_PATH)
  return data.data
}

export const getObjectById = async (id: string): Promise<UpcycleObject> => {
  const { data } = await apiUpcycle.get<ApiResponse<UpcycleObject>>(`${BASE_PATH}/${id}`)
  return data.data
}

export const createObject = async (payload: ObjectPayload): Promise<UpcycleObject> => {
  const { data } = await apiUpcycle.post<ApiResponse<UpcycleObject>>(BASE_PATH, payload)
  return data.data
}
...
export const updateObject = async (
  ...
  const { data } = await apiUpcycle.put<ApiResponse<UpcycleObject>>(`${BASE_PATH}/${id}`, payload)
  return data.data
}
...
export const getObjectScore = async (id: string): Promise<ObjectScore> => {
  const { data } = await apiUpcycle.get<ApiResponse<ObjectScore>>(`${BASE_PATH}/${id}/score`)
  return data.data
}
...
export const getObjectDeliveryMethods = async (id: string): Promise<DeliveryMethodRef[]> => {
  const { data } = await apiUpcycle.get<ApiResponse<DeliveryMethodRef[]>>(
...
export const addObjectDeliveryMethod = async (
  ...
  await apiUpcycle.post(`${BASE_PATH}/${id}/delivery-methods/${deliveryMethodId}`)
}
