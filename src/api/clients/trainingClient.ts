...
export const getTrainings = async (): Promise<Training[]> => {
  const { data } = await apiTraining.get<ApiResponse<Training[]>>(`${BASE_PATH}/`)
  return data.data
}

// Seul `name` est obligatoire.
export const createTraining = async (payload: TrainingPayload): Promise<Training> => {
  const { data } = await apiTraining.post<ApiResponse<Training>>(`${BASE_PATH}/`, payload)
  return data.data
}
...
export const updateTraining = async (
  ...
  const { data } = await apiTraining.put<ApiResponse<Training>>(`${BASE_PATH}/${id}/`, payload)
  return data.data
}

export const deleteTraining = async (id: number): Promise<void> => {
  await apiTraining.delete(`${BASE_PATH}/${id}/`)
}

// --- Sous-ressources ---

export const getTrainingCurricula = async (id: number): Promise<TrainingCurriculum[]> => {
  const { data } = await apiTraining.get<ApiResponse<TrainingCurriculum[]>>(
...
export const getTrainingContent = async (id: number): Promise<TrainingContentRef[]> => {
  const { data } = await apiTraining.get<ApiResponse<TrainingContentRef[]>>(
...
export const getTrainingSchedules = async (id: number): Promise<TrainingSchedule[]> => {
  const { data } = await apiTraining.get<ApiResponse<TrainingSchedule[]>>(
