...
export const getEventSteps = async (): Promise<EventStep[]> => {
  const { data } = await upcycleApiClient.get<ApiResponse<EventStep[]>>(BASE_PATH)
  return data.data
}

export const getEventStepById = async (id: number): Promise<EventStep> => {
  const { data } = await upcycleApiClient.get<ApiResponse<EventStep>>(`${BASE_PATH}/${id}`)
  return data.data
}

export const createEventStep = async (payload: EventStepPayload): Promise<EventStep> => {
  const { data } = await upcycleApiClient.post<ApiResponse<EventStep>>(BASE_PATH, payload)
  return data.data
}
...
export const updateEventStep = async (
  ...
  const { data } = await upcycleApiClient.put<ApiResponse<EventStep>>(`${BASE_PATH}/${id}`, payload)
  return data.data
}
