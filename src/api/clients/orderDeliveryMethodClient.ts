...
export const getOrderDeliveryMethods = async (): Promise<OrderDeliveryMethod[]> => {
  const { data } = await upcycleApiClient.get<ApiResponse<OrderDeliveryMethod[]>>(BASE_PATH)
  return data.data
}
...
export const createOrderDeliveryMethod = async (
  ...
  const { data } = await upcycleApiClient.post<ApiResponse<OrderDeliveryMethod>>(BASE_PATH, payload)
  return data.data
}
...
export const deleteOrderDeliveryMethod = async (
  ...
  await upcycleApiClient.delete(`${BASE_PATH}/${orderId}/${deliveryMethodId}`)
}
